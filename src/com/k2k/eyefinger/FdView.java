...
class FdView extends SampleCvViewBase {
	...
	private Mat mResult;
	private Mat teplateR; // 로컬 변수 사용해도 될 것 같음
	private Mat teplateL; // 로컬 변수 사용해도 될 것 같음
	...
	public FdView(Context context) {
		...
		try {
			...
			InputStream iser = context.getResources().openRawResource(R.raw.haarcascade_righteye_2splits);
			File cascadeDirER = context.getDir("cascadeER", Context.MODE_PRIVATE);
			// File cascadeFileER = new File(cascadeDirER,
			// "haarcascade_eye_right.xml");
			File cascadeFileER = new File(cascadeDirER, "haarcascade_lefteye_2splits.xml");
			...
			File cascadeFileEL = new File(cascadeDirEL, "haarcascade_lefteye_2splits.xml");
	...
	protected Bitmap processFrame(VideoCapture capture) {
		...
		if (mDetectorType == JAVA_DETECTOR) {
			...
			for (int i = 0; i < facesArray.length; i++) {
				...
				eyearea = new Rect(r.x + r.width / 8, (int) (r.y + (r.height / 4.5)), r.width - 2 * r.width / 8,
						(int) (r.height / 3.0));
				...
				if (learn_frames < 5) {
					...
					// todo : 여기서 무엇을 하는 것 같음!!! 나중에 복구 해볼 것!!!!!!
					match_value = match_eye(eyearea_right, teplateR, TM_SQDIFF_NORMED);//FdActivity.method);
					match_value = match_eye(eyearea_left, teplateL, TM_SQDIFF_NORMED);//FdActivity.method);
					// ;
				}
